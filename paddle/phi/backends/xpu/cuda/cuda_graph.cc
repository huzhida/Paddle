...
#include "paddle/phi/backends/xpu/cuda/cuda_graph.h"
...
CUDAGraphID CUDAGraph::ID() const {
  return id_;
}

int64_t CUDAGraph::PoolID() const {
  return pool_id_;
}
...
void CUDAGraph::Replay() {
  ...
  for (size_t i = 0; i < n; ++i) {
    ...
    PADDLE_ENFORCE_XPU_SUCCESS(cudaGraphLaunch(exec_graphs_[i], stream_));
  }
  is_first_run_ = false;
#endif
}
...
void CUDAGraph::AddPostResetCallback(CUDAPostResetCallback callback) {
  std::lock_guard<std::mutex> guard(mtx_);
  cudagraph_post_reset_callbacks_.push_back(std::move(callback));
}

void CUDAGraph::AddPreCaptureCallback(CUDAPreCaptureCallback callback) {
  cudagraph_pre_capture_callbacks_.push_back(std::move(callback));
}

void CUDAGraph::AddPostCaptureCallback(CUDAPostCaptureCallback callback) {
  std::lock_guard<std::mutex> guard(mtx_);
  cudagraph_post_capture_callbacks_.push_back(std::move(callback));
}

void CUDAGraph::AddJoiningStream(cudaStream_t stream) {
  streams_to_join_.insert(stream);
}

void CUDAGraph::PrintToDotFiles(const std::string &dirname, unsigned int flags){
  ThrowErrorIfNotSupportCUDAGraph();
...
int64_t CUDAGraph::SetMemoryPoolID(int64_t pool_id) {
...
int64_t CUDAGraph::CapturingPoolID() {
...
void CUDAGraph::AddJoiningStreamDuringCapturing(cudaStream_t stream){
